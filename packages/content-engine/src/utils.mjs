...
export function writeFile(fp, content) {
...
// Simple sanitizer so injected strings never break TSX
function safeText(s) {
  return String(s ?? "").replace(/[\r\n]/g, " ").replace(/"/g, '\\"');
}

export function toolComponentSource({ title, description, fields, compute }) {
...
const FIELDS: Field[] = ${JSON.stringify(fields || [], null, 2)};
...
export default function Tool() {
  ...
  const result = useMemo(() => {
    ...
    const compute = "${safeText(compute || "")}";
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <Card>
        <CardHeader>
          <h1 className="text-2xl font-semibold">${JSON.stringify(safeTitle)}</h1>
          <p className="text-sm text-gray-600 mt-2">${JSON.stringify(safeDesc)}</p>
        </CardHeader>
